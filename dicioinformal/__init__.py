...
def definicao(query):
	...
	for i in title:
		try:
			b = {'title':i.replace('\t',''),'tit':tit[max].replace('\t',''),'desc':desc[max].replace('\t','').replace('                ','')}
			max += 1
			result.append(b)
		except:
			pass
...
def sinonimos(query):
	r = requests.get(link+'sinonimos/'+query)
	soup = BeautifulSoup(r.text, "html.parser")
...
def antonimos(query):
	r = requests.get(link+'antonimos/'+query)
	soup = BeautifulSoup(r.text, "html.parser")
	a = soup.find('p','text-justify')
	result = []
	for i in a.find_all('a'):
...
def relacionadas(query):
	r = requests.get(link+'relacionadas/'+query)
	soup = BeautifulSoup(r.text, "html.parser")
	a = soup.find('p','text-justify')
	result = []
	for i in a.find_all('a'):
...
def rimas(query):
	r = requests.get(link+'rimas/'+query)
	soup = BeautifulSoup(r.text, "html.parser")
	a = soup.find_all('a')
	b = []
	for i in a:
